...
fun translateForExpression(expression: KtForExpression, context: TranslationContext): JsStatement {
    ...
    fun extractWithIndexCall(): WithIndexInfo? {
        ...
        val receiverType = when {
            ...
            KotlinBuiltIns.isCollectionOrNullableCollection(receiverClass.defaultType) -> WithIndexReceiverType.ITERABLE
            KotlinBuiltIns.isIterableOrNullableIterable(receiverClass.defaultType) -> WithIndexReceiverType.COLLECTION
            else -> return null
        }
    ...
    fun translateForOverCollectionWithIndex(info: WithIndexInfo): JsStatement {
        ...
        val iterableDescriptor = findIterable()

        val iteratorVar = JsScope.declareTemporary()
        val iteratorDescriptor = getFunctionByName(iterableDescriptor.unsubstitutedMemberScope, Name.identifier("iterator"))
